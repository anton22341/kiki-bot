...
### Сделано (продолжение)
- Вариант A: блок показывает строки "Сейчас 172 → avg 106 🟢 +62%" ✅
- Баг benchmark с накопительными данными — исправлен ✅
- Уведомления при вводе данных (22:00–08:00) → superadmin+owner ✅
  Формат: время · дата, внутри+тренд, этот час с бенчмарком, итог ночи
  get_benchmark теперь вычисляет дельту для накопительных ночей
  вместо использования сырых накопленных значений
